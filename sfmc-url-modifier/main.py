...
"""
SFMC URL Modifier
Modification en masse des URLs dans les emails transactionnels
"""
...
def list_journeys(api, status_filter=None, transac_only=True):
    header("Journeys Transactionnelles" if transac_only else "Toutes les Journeys")

    data = api.get_journeys(status_filter=status_filter)
    all_j = data.get('items', [])

    if transac_only:
        journeys = [j for j in all_j if j.get('definitionType') == 'Transactional']
    else:
        journeys = all_j
...
def extract_asset_id(activity):
    """Extrait l'asset ID de la config d'une activity"""
...
def get_journey_ids(api, journey_arg, all_transac):
    """Récupère les IDs à traiter"""
    if all_transac:
        info("Récupération journeys transactionnelles...")
        data = api.get_journeys()
        transac = [j for j in data.get('items', []) if j.get('definitionType') == 'Transactional']
        ids = [j.get('id') for j in transac]
...
def main():
    p = argparse.ArgumentParser(description='SFMC URL Modifier')
    ...
    p.add_argument('--all-transac', action='store_true', help='Toutes les journeys transac')
    ...
    if args.mode in ['analyze', 'execute', 'scan']:
        if not args.journey_id and not args.asset_id and not args.all_transac:
            err("--journey-id, --asset-id ou --all-transac requis")
            sys.exit(1)

    header("SFMC URL Modifier")
    ...
    try:
        info("Connexion...")
        auth = SFMCAuth()
        auth.refresh()  # Nouvelle session à chaque fois
        api = SFMCAPI(auth)

        results = None

        if args.mode == 'list-journeys':
            results = list_journeys(api, args.status, transac_only=True)

        elif args.mode == 'list-all-journeys':
            results = list_journeys(api, args.status, transac_only=False)
            ...
            if args.asset_id:
                ...
                ids = get_journey_ids(api, args.journey_id, args.all_transac)
                results = []
                for jid in ids:
